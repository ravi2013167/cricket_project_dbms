...
ORDER BY `SIX` DESC";
				...
				<tr>
						...
						<table class='table' width=90% height=90% style="text-align:center;">
							...
							<?php
							$qry="SELECT `PLAYER_NAME`,players.PLAYER_ID,SUM(`RUN_SCORED`), SUM(`RUN_SCORED`)/SUM(BALL_PLAYED)*100 AS SR,SUM(BALL_PLAYED),TEAM_ID FROM plays_in,players WHERE MATCH_ID='".$m_id."' AND  plays_in.PLAYER_ID=players.PLAYER_ID GROUP BY `PLAYER_NAME`
ORDER BY `SR` DESC";
							$result=mysql_query($qry);$row=mysql_fetch_assoc($result);
							$f = sprintf ("%.2f", $row['SR']);
							...
							echo "<tr><td width=100% colspan=2><table  width=100% height=100%  style='text-align:center;'>";
							$qry="SELECT SUM(OVER_BOWLED) AS OB FROM plays_in WHERE MATCH_ID='".$m_id."'  AND PLAYER_ID LIKE '".$in."%'";
							$result=mysql_query($qry);
							$row=mysql_fetch_assoc($result);
							...
							$qry="SELECT SUM(OVER_BOWLED) AS OB FROM plays_in WHERE MATCH_ID='".$m_id."'  AND PLAYER_ID LIKE '".$in2."%'";
							$result=mysql_query($qry);
							$row=mysql_fetch_assoc($result);
							echo "<tr class='hide'></tr>";
...
ORDER BY W DESC";
					...
					<td width=33.33%>
						<table class='table' width=90% height=90% style="text-align:center;">
							...
							<?php
							$qry="SELECT `PLAYER_NAME`,players.PLAYER_ID,SUM(RUN_SCORED)/SUM(OVER_BOWLED)/6 AS EC,TEAM_ID FROM plays_in,players WHERE MATCH_ID='".$m_id."' AND  plays_in.PLAYER_ID=players.PLAYER_ID AND OVER_BOWLED>0 GROUP BY PLAYER_NAME ORDER BY EC";
							$result=mysql_query($qry);$row=mysql_fetch_assoc($result);
							...
							$pak_bowl=$row['OB'];
							if($pak_bowl==0)$pak_bowl=1;
							$e=round((($pak_score*6)/$ind_bowl),2);
							...
							$e=round((($ind_score*6)/$pak_bowl),2);
