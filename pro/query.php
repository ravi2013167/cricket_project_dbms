...
<html>
	...
	<body id="body">
		<div id="container1">
			...
			?>
			<div id='sq'>
				<table width=100% height=90% style="text-align:center;vertical-align:top;">
				<?php
			...
			//	$qry="SELECT (SUM(RUN)+SUM(EXTRA)) AS RUNS,(((OVER-1)-(OVER-1)%10)/10) AS OVER FROM `balls` WHERE OVER BETWEEN 0 AND 30 AND MATCH_ID='M1' GROUP BY (((OVER-1)-(OVER-1)%10)/10)";
				$qry="SELECT *,(((OVER-1)-(OVER-1)%10)/10) AS O FROM balls WHERE BATSMAN LIKE '".$_GET['ing']."%' AND MATCH_ID='".$_GET['m_id']."' AND OVER BETWEEN ".$f." AND ".$l."";
				...
				$bl++;
				$i=1;
				echo "<tr><td style='width:33.33%;text-align:center;vertical-align:top;'><table class='table' width=100% height=50px>
					<tr class='trs'>
				...
				while($row=mysql_fetch_assoc($result))
				{
					...
					if($b!=$row['BOWLER'])
					{
						...
						<tr><td colspan=2 height=8% style='background-color: #2b2948;font-size:25px'><strong>";
						$runs+=$r;
					$extra+=$e;
					$wk+=$w;
					$four+=$fr;
					...
					$ball+=$bl;
						if($s=='runs')
						{echo "Runs - ".($r+$e)."</strong></td>
						</tr></table></td>";}
						
						else if ($s=='fours')
						{
							echo "Fours - ".($fr)."</strong></td>
						</tr></table></td>";
						}
						else if ($s=='sixes')
						{
							echo "Sixes - ".($sx)."</strong></td>
						</tr></table></td>";
						}
						else if ($s=='strike_rate')
						{
							echo "Strike Rate - ".round(((($runs)/$ball)*100),2)."</strong></td>
						</tr></table></td>";
						}
						else if ($s=='wickets')
						{
							echo "Wicket - ".($w)."</strong></td>
						</tr></table></td>";
						}
						else if($s=='economy')
						{
							echo "Run Rate - ".round(((($runs)*6)/$ball),2)."</strong></td>
						</tr></table></td>";
						}
					...
					echo "<td style='width:33.33%;text-align:center;vertical-align:top;'><table  class='table' width=100% height=100px style='text-align:center;vertical-align:top;'>
					<tr class='trs'>
					<td style='background-color: #eac80d;font-size:30px;text-align:center;vertical-align:top;' colspan=2 height=10%><strong>Over ".(((int)$row['O'])+1)."</strong>
					</td>
					</tr>";
				...
				if($s=='runs')
						...
						echo "<h2> Runs Scored of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']. " overs is ".$runs. " in which ".$extra." runs were extras</h 2>";
						}
						else if ($s=='fours')
						{
							echo "Fours - ".($fr)."</strong></td>
						</tr></table></td>";
						echo "<h2>There were ".$four." Fours of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs</h2>";
						}
						else if ($s=='sixes')
						{
							echo "Sixes - ".($sx)."</strong></td>
						</tr></table></td>";
						echo "<h2>There were ".$six." Sixes of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs</h2>";
						}
						else if ($s=='strike_rate')
						{
							echo "Strike Rate - ".round(((($runs)/$ball)*100),2)."</strong></td>
						</tr></table></td>";
						echo "<h2> Strike Rate of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs  is ".round((($runs/$ball)*100),2)."</h2>";
						}
						else if ($s=='wickets')
						{
							echo "Wicket - ".($w)."</strong></td>
						</tr></table></td>";
						echo "<h2> Wickets fallen of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs were ".$wk."</h2>";
						...
						echo "<h2> Runs Rate of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs is ".round(((($runs+$extra)*6)/$ball),2)."</h2>";
						}
				?>
				</tr></table>
			</div>	
