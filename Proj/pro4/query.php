			...
			//	$qry="SELECT (SUM(RUN)+SUM(EXTRA)) AS RUNS,(((OVER-1)-(OVER-1)%10)/10) AS OVER FROM `balls` WHERE OVER BETWEEN 0 AND 30 AND MATCH_ID='M1' GROUP BY (((OVER-1)-(OVER-1)%10)/10)";
				...
				while($row=mysql_fetch_assoc($result))
				{
					...
					$ball+=$bl;
						...
						{
							echo "Strike Rate - ".((($runs+$extra)/$ball)*100)."</strong></td>
						</tr></table></td>";
						}
						else if ($s=='wickets')
						{
						...
						{
							echo "Run Rate - ".((($runs+$extra)*6)/$ball)."</strong></td>
						</tr></table></td>";
						}
				...
				if($s=='runs')
						...
						echo "<h2> Strike Rate of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs is ".((($runs+$extra)/$ball)*100)."</h2>";
						}
						else if ($s=='wickets')
						{
							echo "Wicket - ".($w)."</strong></td>
						...
						echo "<h2> Runs Rate of ".$_GET['ing']." from ".$_GET['over_start'].".".$_GET['ball_start']." to ".$_GET['over_end'].".".$_GET['ball_end']." overs is ".((($runs+$extra)*6)/$ball)."</h2>";
						}
				?>
				</tr></table>
			</div>	
