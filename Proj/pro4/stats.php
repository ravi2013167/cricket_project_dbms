...
ORDER BY W DESC";
					...
					<td width=33.33%>
						<table class='table' width=90% height=90% style="text-align:center;">
							...
							$result=mysql_query($qry);$row=mysql_fetch_assoc($result);
							echo "<tr  height=15%><td style='background-color: #2b2937;font-size:20px' width=50%>";echo $row['PLAYER_ID'][0].$row['PLAYER_ID'][1].$row['PLAYER_ID'][2].'<br><br>';echo $row['PLAYER_NAME'];echo "<br><br>";echo $row['EC'];$img = 'images/'.$row['PLAYER_ID'].'.jpg';echo "</td><td colspan=2 style='background-color: #2b2937;font-size:20px'><img src=$img /></td></tr>";
							$i=0;while($i<5)
							{
								$row=mysql_fetch_assoc($result);
								$f=sprintf ("%.1f", $row['EC']);
				...
				<tr>
					<td width=33.33%>
						<table class='table' width=90% height=90% style="text-align:center;">
							...
							<?php
							$qry="SELECT PLAYER_NAME,players.PLAYER_ID,TEAM_ID,COUNT(*) AS CT FROM plays_in,players WHERE (SUBSTR(OUTS,11,14))=players.PLAYER_ID AND plays_in.`PLAYER_ID` LIKE 'I%' AND OUTS LIKE '%c %' GROUP BY (SUBSTR(OUTS,11,14))";
							$result=mysql_query($qry);$row=mysql_fetch_assoc($result);
				...
				<tr>
					<td width=33.33%>
						<table class='table' width=90% height=90% style="text-align:center;">
							...
							{
								$row=mysql_fetch_assoc($result);
								echo "<tr><td>".$row['PLAYER_NAME']."</td><td>".$row['TEAM_ID']."</td><td>".$row['AVE']."</td></tr>";
								$i++;
							}
